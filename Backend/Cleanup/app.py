...
#         resp = byType.to_dict('record')
    ...
    ### Connection to database ###
    connect(airport_name,data,db)
#     print(data)

    ### Updating Meta and checking for airport Details ###
    check_meta(airport_name,exp_index_till_date,dates,generals,db)

    ### Updating the Meta function ###
def check_meta(airport_name, expData, date,generals,db):
    total = 0
    c = 0
    mycol = db["Meta"]
...
def connect(airport_name,data,db):
