...
const air_resolve = async (parent, args) => {
  let res = {},
    filter = {};
  if (args.code) filter.code = args.code;
  else filter.name = args.name;
  await db
    .getDB()
    .collection("Meta")
    .findOne(filter, {
    ...
    device_list: {
      ...
      async resolve(parent, args) {
        ...
        await db
          ...
          .findOne(
            { _id: "device" },
            { projection: { device_names: 1, _id: 0 } }
          )
          .then((doc) => {
    ...
    device_name: {
      ...
      async resolve(parent, args) {
        ...
        await db
          ...
          .collection(args.air)
          .findOne(
            { date: args.date, type: args.name },
            { projection: { _id: 0, type: 1, general: 1 } }
          )
          .then((doc) => {
