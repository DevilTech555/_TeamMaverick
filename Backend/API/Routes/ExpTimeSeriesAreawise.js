...
const {ExpTS} = require('../Meta/ExpTimeSeriesAreawise.json');
...
router.get('/:air/exp_series/:sec',(req,res)=>{
    ...
    db.getDB().collection(req.params.air).find({date: {$gte:req.query.sdate,$lt:req.query.edate}, type: req.query.type}).project(db_query).toArray((err,documents)=>{
        if(err){
         ...
         res.status(200).render("chart_exp_timeseries",{option: JSON.stringify(ExpTS),series: JSON.stringify(series), area: JSON.stringify(area)});                                    
        }
    })
});
